...
- **Execution smoke test:** Use when the question is whether the skill activates or executes its workflow mechanics: reading dependencies, spawning the required agents, retaining identities, invoking tools, producing artifacts, respecting authority, or failing correctly. In temporary staged copies, substitute native Codex workers and reviewers with `gpt-5.6-luna` at `low` effort and `priority` service tier, and substitute Claude runs—including the independent judge—with `haiku` at `low` effort. Apply the same substitutions to both arms. Record every substitution and do not claim that production-model output quality was evaluated.
- **Quality evaluation:** Use when judging architecture, review accuracy, synthesis, instruction quality, model choice, model provenance, or any result whose correctness depends materially on reasoning quality. Run the skill's actual declared models, efforts, and service tiers. Use this profile whenever the distinction is uncertain.

Never change an installed skill to obtain the smoke-test profile. Apply substitutions only to complete temporary skill and dependency snapshots plus their launch arguments. Cleanup removes those snapshots, leaving the installed production configuration unchanged. Do not substitute the model when model selection, effort, service tier, alias resolution, or verified provenance is itself under evaluation.
...
- **Orchestration candidate:** when the target skill must itself spawn native Codex agents, launch the staged candidate as a fresh top-level `codex exec --ephemeral --json` process in its disposable Git repository. The evaluator owns this process boundary; the candidate must use its native agent tools for delegation and must not launch another Codex process. Pass the fidelity profile's exact candidate model and effort on the command line.

The `codex exec` path evaluates native agent behavior within that isolated candidate session. It does not prove integration with the calling Codex desktop task, whose agent tree cannot address the candidate's children. Do not use it when that app-level relationship is itself under evaluation.
...
Use [Claude Code](../claude-code/SKILL.md) for the independent blinded judge. For an execution smoke test, use Claude Haiku at `low` effort. For a quality evaluation, use Claude Fable 5.1 (`claude-fable-5-1`) at `xhigh`. Give the judge the rubric, neutral labels, complete outputs, captured traces, and artifact paths, but not model identities or which arm is current. For a revision, both arms must be judged together in one pass on one scale. For a current-skill evaluation, judge the single run against the declared contract and ask for concrete issues.

Before launching Claude, create one common temporary evidence root containing the rubric, neutral candidate outputs, traces, and every artifact the judge must inspect. Resolve every referenced path and verify that it is beneath either this evidence root or the Codex skills directory exposed by the Claude launcher. Copy required evidence into the common root under neutral names when it is elsewhere; fail the judge preflight rather than launching with unreadable evidence. Run Claude with the common evidence root as its working directory. Put the launcher's output directory outside that readable root so model provenance, process logs, and other parent-only metadata cannot compromise the blind judgment. Pass absolute paths and preserve the preflight result in the evaluation evidence.

If the selected Claude judge is unavailable, preserve the capability failure and let the parent perform the rubric-level judgment; label the evaluation degraded. Do not silently substitute another judge.
