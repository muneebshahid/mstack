...
Read [references/candidate-prompt.md](references/candidate-prompt.md) and prepare one self-contained base prompt. All default candidates receive the same task, evidence, constraints, and output contract. Read [MStack runtime model resolution](../setup-mstack/references/runtime-resolution.md) completely and resolve `arena_candidate_a`, `arena_candidate_b`, and `arena_cross_judge`. Process-boundary instructions may differ because configured runners can differ.
...
Read [references/judge-prompt.md](references/judge-prompt.md). Launch `arena_cross_judge` through its resolved runner with no inherited conversation history and a read-only, no-delegation role. Retain its process or agent identifier for monitoring and completion.

Give the cross-judge the original task, rubric, every complete candidate labelled neutrally, and all candidate capability issues. Include each complete report directly or provide an exact readable temporary path. The judge scores every criterion, recommends a base, identifies compatible grafts and conflicts, and drafts a synthesis plan. Candidate labels must not reveal model names until judging is complete.

While the cross-judge works, the parent reads every candidate end to end and scores it independently against the same rubric.

Capture the cross-judge's complete final report through its configured result boundary and close a native agent after preserving it. Recheck the repository audit state after judging. If the judge fails, inspect the failure once, preserve the blocker, and let the parent complete Pick and Graft without a cross-judge; label the Arena degraded. If the judge unexpectedly mutates the repository, report the mutation and exclude its judgment.

## 4. Pick

The parent retains final lead judgment.

1. Compare its criterion-level scores with the cross-judge's judgment.
...
Agreement with the cross-judge strengthens the choice but does not replace parent judgment. Record why the base won and why each other whole shape lost.
...
The parent produces the final synthesized artifact in the response. The cross-judge's synthesis plan is advice, not the deliverable. Arena does not create repository files or apply the artifact to project code.
