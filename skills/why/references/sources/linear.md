...
Use the callable Linear connector read-only. In Codex, resolve and read the available Linear skill from the advertised skill roots, then attach the skill or connector to the investigator when the harness supports structured mentions. If the connector is available only to the parent, accept parent-mediated query results and disclose that access path. Never create or update issues, comments, projects, or documents during a Why investigation.
