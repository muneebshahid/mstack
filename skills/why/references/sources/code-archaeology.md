...
Keep retrieval bounded. Fetch PR metadata and body first, then comments and reviews only for the most relevant PRs. Avoid full repository-wide diffs or giant undifferentiated payloads when targeted file history, selected PR fields, and specific review threads can answer the question. Preserve enough time to return the investigator report.
