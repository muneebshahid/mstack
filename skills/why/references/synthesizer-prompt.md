...
You MUST read and follow the framework at `{EPISTEMICS_PATH}` in full before writing the output. The key rules:
...
7. **Report capability failures.** If a tool, MCP server, plugin, skill, file, permission, authentication, or command is unavailable or fails, preserve the failure as a gap. Continue with available evidence when possible, avoid repeated retries, and distinguish failure from a successful search with no relevant result.
...
- **Source control history**: {file paths}, {number of commits reviewed}, PRs #{numbers}, and code comments searched. Distinguish local git coverage from GitHub PR coverage when `gh` is unavailable.
...
Omit this section when no capability or tool issue occurred. Otherwise list every issue with:

- The capability or source and attempted operation
- A concise observed error with credentials and secrets redacted
- The evidence or verification affected
- Its impact on completeness or confidence
- A useful next diagnostic, authentication, permission, or setup step

Preserve capability issues reported by investigators even if your own tools work. State when a source was accessed through the parent rather than directly by its investigator.
...
8. Did you preserve every material capability or tool failure and distinguish it from a searched source with no results?
