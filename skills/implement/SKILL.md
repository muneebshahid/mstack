...
Use one entry point for code-changing work. The parent agent remains the lead: it chooses the mode, owns scope and judgment, decomposes the work, inspects the actual diff, verifies each unit, and decides whether the evidence is strong enough to finish. A persistent Luna implementation worker writes project code.
...
1. Pass the load-bearing decisions, adaptable details, unresolved uncertainties, and invalidation signals to the Luna worker.
