...
## Required Validation and Identity
...
Deduplicate only findings with the same causal defect and affected execution path. Preserve distinct causes even when symptoms match. After deduplication, assign stable `INT-NNN` IDs and retain reviewer-local IDs and model attribution beneath them.
