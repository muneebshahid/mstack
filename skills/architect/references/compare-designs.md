...
Resolve `architect_candidate_a` and `architect_candidate_b` through [model configuration](../../setup-mstack/references/runtime-resolution.md). Use [Consult](../../consult/SKILL.md) for external candidates. Record the current commit, status, and diff before launching.
...
Launch the two candidates concurrently in fresh contexts. Retain their identifiers, monitor completion, and capture reports and available model provenance. Keep conversations for useful follow-ups. Keep external artifacts outside the repository.
