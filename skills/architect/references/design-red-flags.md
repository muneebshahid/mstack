...
Screen every candidate before synthesis. A red flag is a reason to revise or reject the shape, not a new principle.

## Shallow module

A shallow module exposes a large interface while hiding little complexity. Judge depth by the capability and policy hidden behind the public surface relative to its size. Prefer a simple interface backed by substantial behavior.
